...
# ==========================================
# 1. PURE PYTHON CRYPTOGRAPHY (SHA-256)
# ==========================================
...
def _shr(x, n): return (x >> n) & 0xFFFFFFFF
def _ch(x, y, z): return (x & y) ^ ((x ^ 0xFFFFFFFF) & z)
...
def sha256(message_string):
    ...
    b.append(0x80)
    while len(b) % 64 != 56: b.append(0x00)
    b += orig_len_bits.to_bytes(8, 'big')
    h = list(H_INIT)
    for chunk_start in range(0, len(b), 64):
        chunk = b[chunk_start:chunk_start+64]
        w = [0] * 64
        for i in range(16): w[i] = int.from_bytes(chunk[i*4:i*4+4], 'big')
        for i in range(16, 64): w[i] = (_sigma1(w[i-2]) + w[i-7] + _sigma0(w[i-15]) + w[i-16]) & 0xFFFFFFFF
        a, b_reg, c, d, e, f, g, h_reg = h
        for i in range(64):
            t1 = (h_reg + _SIGMA1(e) + _ch(e, f, g) + K[i] + w[i]) & 0xFFFFFFFF
            t2 = (_SIGMA0(a) + _maj(a, b_reg, c)) & 0xFFFFFFFF
            h_reg, g, f, e, d, c, b_reg, a = g, f, e, (d + t1) & 0xFFFFFFFF, c, b_reg, a, (t1 + t2) & 0xFFFFFFFF
        h = [(h[i] + val) & 0xFFFFFFFF for i, val in enumerate((a, b_reg, c, d, e, f, g, h_reg))]
    return ''.join('{:08x}'.format(value) for value in h)

# ==========================================
# 2. PURE PYTHON RSA & WALLETS
# ==========================================

class SHA256_PRNG:
    def __init__(self, seed_text): self.state = sha256(seed_text)
...
class Transaction:
    ...
    def calculate_hash(self):
        return sha256(str(self.sender) + str(self.recipient) + str(self.amount))

    def sign_transaction(self, private_key):
        if self.sender == "System": return
        self.signature = pow(int(self.calculate_hash(), 16), private_key[0], private_key[1])

    def is_valid(self):
        if self.sender == "System": return True
        if self.signature is None: return False
        return int(self.calculate_hash(), 16) == pow(self.signature, self.sender[0], self.sender[1])
...
class Block:
    ...
    def calculate_hash(self):
        ...
        return sha256(str(self.index) + str(self.timestamp) + tx_string + str(self.previous_hash) + str(self.nonce))
    ...
    def to_dict(self):
        return {"index": self.index, "timestamp": self.timestamp, "transactions": [tx.to_dict() for tx in self.transactions],
...
class Blockchain:
    ...
    def add_transaction(self, transaction):
        ...
        if transaction.sender != "System" and self.get_balance_of_address(transaction.sender) < transaction.amount: return False
        self.pending_transactions.append(transaction)
        return True

    def mine_pending_transactions(self, mining_reward_address):
        self.pending_transactions.append(Transaction("System", mining_reward_address, self.mining_reward))
    ...
    def to_dict(self):
        return {"chain": [b.to_dict() for b in self.chain], "difficulty": self.difficulty, "time_simulator": self.time_simulator}
    ...
    def save_to_disk(self):
        with open(self.storage_file, "w") as f: f.write(str(self.to_dict()))
...
class Node:
    ...
    def handle_client(self, conn):
        ...
        if command == "GET_CHAIN":
            conn.sendall(("CHAIN_REPLY|" + str(self.blockchain.to_dict())).encode('utf-8'))
            
        elif command == "NEW_TX":
            tx = Transaction.from_dict(eval(parts[1]))
...
if __name__ == "__main__":
    ...
    print("=========================================")
    print(" 1. Start Node A in Terminal 1 (Port 5000)")
    print(" 2. Start Node B in Terminal 2 (Port 5001)")
    print("=========================================\n")
    
    choice = input("Start as Node (1 for Node A, 2 for Node B): ")
    print("\nGenerating deterministic cryptography keys...")
    alice = Wallet("Alice")
    bob = Wallet("Bob")
    
    if choice == "1":
        print("--- STARTING NODE A (MINER ON PORT 5000) ---")
        my_coin = Blockchain("chain_A.dat")
        ...
        print("--- STARTING NODE B (PEER ON PORT 5001) ---")
        my_coin = Blockchain("chain_B.dat")
        ...
        print(f"\nNode B Final Balance: {my_coin.get_balance_of_address(bob.public_key)} coins")
    else:
